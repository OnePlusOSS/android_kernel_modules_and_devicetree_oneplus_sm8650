...
void onConfigureMvdMeta_i7(u32 count, u32 *values)
{
	...
	IRIS_LOGI("i7:SET_MVD_META: %d: [%x][%d, 0x%04x%04x, %d, %d][%d]",
		count, values[0],
		values[1], values[2], values[3], values[4], values[5],
		values[16]);
	IRIS_LOGI("iris7: SET_MVD_META:[%d, 0x%04x%04x, %d, %d][%d, 0x%04x%04x, %d, %d]",
	...
	if (count >= 19) {
		...
		IRIS_LOGI("i7:SET_MVD_META:bmvSize %d,ori:%d,gmvd: format %d,"
